...
_RAISE_MIN_STR = {'pre': 0.6, 'post': 0.65}
_CALL_MIN_STR = {'pre': 0.55, 'post': 0.6}
...
class Player(Bot):
        ...
        if round_num % mutate_iters == 0:
            if self.total_payoffs < 6 * mutate_iters:
                if not mutated:
                    ...
                    draise_pre = random.random() * 0.05 - 0.02
                    draise_post = random.random() * 0.05 - 0.02
                    dcall_pre = random.random() * 0.05 - 0.02
                    dcall_post = random.random() * 0.05 - 0.02

                    _RAISE_MIN_STR['pre'] += draise_pre
                    _RAISE_MIN_STR['post'] += draise_post
                    _CALL_MIN_STR['pre'] += dcall_pre
                    _CALL_MIN_STR['post'] += dcall_post
                else:
                    mutated = False

                    _RAISE_MIN_STR['pre'] -= 2 * draise_pre
                    _RAISE_MIN_STR['post'] -= 2 * draise_post
                    _CALL_MIN_STR['pre'] -= 2 * dcall_pre
                    _CALL_MIN_STR['post'] -= 2 * dcall_post
            
            else: # protect from undoing good mutation; set mutated to False
                mutated = False
    ...
    def handle_round_over(self, game_state, terminal_state, active):
        ...
        if round_num == NUM_ROUNDS:
            print(game_clock)
            # print('final raise min str:', _RAISE_MIN_STR)
            # print('final call min str:', _CALL_MIN_STR)
            ...
            if AssignAction in legal_actions[i]:
                ...
                if street < 3: # pre-flop
                    ...
                    if strength > _RAISE_MIN_STR[state_str]:
                        ...
                        raise_amount = int(my_pips[i] + board_cont_cost + street**2 * math.exp(1.8 * (strength - 0.321875)) - 12.5)
                    else:
                        raise_amount = 0 # min_raise
