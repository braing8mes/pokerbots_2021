...
def calculate_strength(hole, iters):
        ...
        for _ in range(iters): # take 'iters' samples
            ...
            my_hand = hole_cards + community # the two showdown hands
            opp_hand = opp_hole + community

            my_hand_value = eval7.evaluate(my_hand) # the ranks of our hands (only useful for comparisons)
            opp_hand_value = eval7.evaluate(opp_hand)

            if my_hand_value > opp_hand_value: # we win!
                score += 2
            
            elif my_hand_value == opp_hand_value: # we tie.
                score += 1
