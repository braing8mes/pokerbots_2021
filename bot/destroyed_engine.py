...
sys.path.append('../../AP_Poker')
import player
sys.path.append('/AP_Poker_copy')
import player
...
class Game():
    ...
    def run(self):
        ...
        players[1].raise_pre += temp[0]
        players[1].raise_post += temp[1]
        players[1].call_pre += temp[2]
        players[1].call_pre += temp[3]
        players[0].raise_pre += mutation
        ...
        if players[0].bankroll > players[1].bankroll:
            print('Alice wins so Alice survives natural selection')
            

        else:
            print('Bob wins so Bob survives natural selection')
...
if __name__ == '__main__':
    temp = [0, 0, 0, 0]
    for x in range(5):
        mutation = 0.05/(x+1) * (random.random()-0.5)
