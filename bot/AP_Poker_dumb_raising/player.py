...
class Player(Bot):
    ...
    def assign_holes(self, hole_cards):
        ...
        if random.random() < 0.25: # swap strongest with second, makes our strategy non-deterministic!
            temp = holes_and_strengths[2]
            holes_and_strengths[2] = holes_and_strengths[1]
            holes_and_strengths[1] = temp
        
        if random.random() < 0.25: # swap second with last, makes us even more random
    ...
    def public_eval(self, private, public, street, iters):
        ...
        # deck = eval7.Deck()
        # private_cards = [eval7.Card(card) for card in private]
        # public_cards = [eval7.Card(card) for card in public]
        ...
        # print('public:', public)
        # print('pooled:', private + public)
        # print('remaining:', remaining)
        # raise NotImplementedError("brian sucks at cs")
        ...
        for _ in range(iters):
            ...
            # deck.shuffle()
            # draw = deck.peek(_OPP + _PUB)
            # opp_hole = draw[:_OPP]
            # hidden_public = draw[_OPP:]

            # my_hand = private_cards + hidden_public # the two showdown hands
            # opp_hand = opp_hole + hidden_public

            # my_strength = eval7.evaluate(my_hand) # the ranks of our hands (only useful for comparisons)
            ...
            if AssignAction in legal_actions[i]:
                ...
                if street < 3: # pre-flop
                    ...
                    if self.starting_strengths[self.hole_list_to_key(cards)] > 0.5:
                        raise_amount = int(my_pips[i] + board_cont_cost + math.sqrt(strength - 0.5) * (pot_total + board_cont_cost)) # play a little conservatively pre-flop
                    else: 
                        raise_amount = 0
                else:
                ...
                if RaiseAction in legal_actions[i] and (raise_cost <= my_stack - net_cost): # raise if we can and if we can afford it
                    commit_action = RaiseAction(raise_amount) # can't raise more than opp_stack, but TODO double check this
                ...
                if board_cont_cost > 0: # our opp raised!!! we must respond
                    ...
                    if board_cont_cost > _INTIMIDATION_THRESHOLD: # <--- parameters to tweak. 
                        intimidation = 0.036 * math.sqrt(board_cont_cost - _INTIMIDATION_THRESHOLD) * math.sqrt(strength)
                    ...
                    if strength >= pot_odds: # Positive Expected Value!! at least call!!
                        if random.random() < 1.4 * strength: # raise sometimes, more likely if our hand is strong
